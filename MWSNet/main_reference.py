...
parser.add_argument('--dataPath', type=str, default='D:/Study/Code/github/SLAM/VPR/pytorch-NetVlad-master/data/', help='Path for centroid data.')
parser.add_argument('--runsPath', type=str, default='D:/Study/Code/github/SLAM/VPR/pytorch-NetVlad-master/runs/', help='Path to save runs to.')
parser.add_argument('--savePath', type=str, default='checkpoints', 
        help='Path to save checkpoints to in logdir. Default=checkpoints/')
parser.add_argument('--cachePath', type=str, default='D:/Study/Code/github/SLAM/VPR/pytorch-NetVlad-master/tmpdir', help='Path to save cache to.')#environ['TMPDIR'],
...
def train(epoch):
    ...
    for subIter in range(subsetN):
        ...
        for iteration, (query, positives, negatives, 
                negCounts, indices) in enumerate(training_data_loader, startIter):#startIter: 从1开始枚举
            # some reshaping to put query, pos, negs in a single (N, 3, H, W) tensor
            # where N = batchSize * (nQuery + nPos + nNeg)
            if query is None: continue # in case we get an empty batch

            B, C, H, W = query.shape
            nNeg = torch.sum(negCounts) # nNeg 负样本数量
            input = torch.cat([query, positives, negatives]) # 输入Q，P，N
            ...
            if iteration % 5 == 0 or nBatches <= 10:
...
def get_clusters(cluster_set):
    ...
    with h5py.File(initcache, mode='w') as h5: 
        with torch.no_grad():
            ...
            for iteration, (input, indices) in enumerate(data_loader, 1):# 序号从1开始
                input = input.to(device)
                ...
                for ix in range(image_descriptors.size(0)):
                    # sample different location for each image in batch  !!!!重点看这里
...
if __name__ == "__main__":
    ...
    if opt.dataset.lower() == 'pittsburgh':
        import pittsburgh as dataset  ### pittsburgh 作为数据集
    else:
    ...
    if opt.mode.lower() == 'train':
        ...
        train_set = dataset.get_training_query_set(opt.margin)### 这里也是只有图片地址
        ...
        whole_train_set = dataset.get_whole_training_set(onlyDB=True) #####这里得到训练集的地址
